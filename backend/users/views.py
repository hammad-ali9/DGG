...
class RegisterView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            # Ensure a Profile record exists for every new user
            Profile.objects.get_or_create(user=user)
            return api_response(True, UserSerializer(user).data, "User registered successfully", status.HTTP_201_CREATED)
...
class MeView(generics.RetrieveUpdateAPIView):
    ...
    def get_object(self):
        ...
        # Guarantee a Profile row exists so nested profile fields never crash
        Profile.objects.get_or_create(user=user)
        return user
